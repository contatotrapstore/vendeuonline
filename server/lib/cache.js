...
const memoryCache = new NodeCache({
  ...
  maxKeys: 500, // Limitar quantidade de chaves para economizar memória
});
  ...
  async invalidatePattern(pattern) {
    try {
      if (this.isRedis && redisClient) {
        ...
        const keys = memoryCache.keys().filter((key) => key.includes(pattern.replace("*", "")));
...
export const CACHE_KEYS = {
  ...
  PRODUCT_SEARCH: (query, filters) => `products:search:${query}:${JSON.stringify(filters)}`,
...
// 🔧 FIX: Cache ultra-rápido para atualizações instantâneas
...
export const cacheMiddleware = (keyGenerator, ttl = CACHE_TTL.MEDIUM, options = {}) => {
  return async (req, res, next) => {
    try {
      // Se skipCacheCondition retornar true, pular cache completamente
      if (options.skipCacheCondition && options.skipCacheCondition(req)) {
        logger.debug("Cache desabilitado por condição:", req.url);
        return next();
      }
