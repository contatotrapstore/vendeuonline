...
export const OPTIMIZED_SELECTS = {
  ...
  PRODUCTS_LIST:
    "id, name, description, price, comparePrice, categoryId, storeId, stock, isActive, isFeatured, salesCount, createdAt, dimensions",

  // Produtos - campos para detalhes
  PRODUCTS_DETAIL:
    "id, name, description, price, comparePrice, categoryId, storeId, stock, isActive, isFeatured, dimensions, seoTitle, seoDescription, salesCount, createdAt, updatedAt",
  ...
  ORDERS_LIST: "id, total, status, paymentStatus, paymentMethod, createdAt, userId, buyerId, storeId",
...
export const RECOMMENDED_INDEXES = {
  ...
  Order: ["userId", "buyerId", "sellerId", "status", "paymentStatus", "createdAt"],
...
/**
 * @deprecated Esta função está obsoleta desde 15/11/2025
 *
 * **LEGACY CODE - NÃO USAR**
 *
 * Busca de produtos agora usa RPC function PostgreSQL (search_products)
 * que é chamada via interceptor em server.js antes de chegar aos routers.
 *
 * Motivo da mudança:
 * - Bug no método .or() do Supabase JS Client v2
 * - ILIKE com .or() não filtrava corretamente
 * - RPC bypassa o bug e funciona 100%
 *
 * Veja:
 * - server.js:348-451 (interceptor RPC)
 * - docs/problems/PROBLEMA-11-BUSCA-NAO-FILTRA.md
 * - docs/reports/SOLUCAO-PROBLEMA-11-RPC-2025-11-15.md
 *
 * Otimização futura:
 * - docs/optimizations/FULL-TEXT-SEARCH-GIN-INDEX.md
 *
 * Esta função pode ser removida em uma futura limpeza de código.
 */
export function applyTextSearch(query, searchTerm, searchFields = ["name"]) {
  ...
  // ATENÇÃO: Este código tem BUG conhecido com .or()
  // Use RPC search_products() ao invés disso
  const cleanTerm = searchTerm.trim();

  // Para múltiplos campos, usa OR com sintaxe PostgREST
  // BUG: .or() não filtra corretamente no Supabase JS v2
  if (searchFields.length > 1) {
    const orConditions = searchFields.map((field) => `${field}.ilike.%${cleanTerm}%`).join(",");
    logger.warn(`⚠️ [LEGACY] applyTextSearch() chamado - considere usar RPC search_products()`);
    logger.info(`🔍 [SEARCH DEBUG] cleanTerm="${cleanTerm}", orConditions="${orConditions}"`);
    query = query.or(orConditions); // ← BUG: não filtra corretamente
  } else {
    query = query.ilike(searchFields[0], `%${cleanTerm}%`); // ← Funciona para 1 campo
  }

  logger.debug(`Busca de texto (LEGACY): ${cleanTerm} em ${searchFields.join(", ")}`);
  return query;
}
