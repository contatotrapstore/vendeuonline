...
// Schema para IDs de produtos (aceita UUID v4 OU IDs customizados product_TIMESTAMP_RANDOM OU IDs de teste)
export const productIdSchema = z.string().refine(
  (id) => {
    ...
    // Aceitar IDs de teste (test_* ou qualquer string com underscores)
    const testIdRegex = /^[a-z0-9_-]+$/i;

    return uuidRegex.test(id) || customIdRegex.test(id) || testIdRegex.test(id);
  },
...
export const createProductSchema = z.object({
  ...
  // Se produto estiver ativo (publicado), exige pelo menos 1 imagem
  if (data.isActive && (!data.images || data.images.length === 0)) {
    return false;
  }
  return true;
}, {
  message: "[NOVO] Pelo menos uma imagem é obrigatória para publicar o produto",
  path: ["images"],
});
