...
export async function getPlans() {
  try {
    return await supabaseFetch("Plan?isActive=eq.true&order=order.asc");
  } catch (error) {
...
export async function getStores() {
  try {
    return await supabaseFetch("Store?isActive=eq.true&select=*,seller:Seller(*,user:User(*))");
  } catch (error) {
...
export async function getTrackingConfigs() {
  try {
    return await supabaseFetch("SystemConfig?category=eq.tracking&isActive=eq.true&select=key,value,isActive");
  } catch (error) {
...
export async function getAdminStats() {
  try {
    const [users, products, stores, orders] = await Promise.all([
      ...
      supabaseFetch("Store?isActive=eq.true&select=id"),
