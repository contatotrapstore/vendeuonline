...
export function GlobalSearch({
  ...
  const performSearch = async (searchQuery: string) => {
    try {
      ...
      // 🔒 FIX (Bug #12 + #13): Use REAL data from Supabase instead of mock data
      ...
      // Get real data from stores after fetch (fetchProducts/fetchStores are void)
      const productState = useProductStore.getState();
      const storeState = useStoreStore.getState();

      // Add REAL product results (not mock)
      if (productState.products && productState.products.length > 0) {
        productState.products.slice(0, MAX_RESULTS_PER_TYPE).forEach((product: any) => {
          searchResults.push({
            id: product.id,
            type: "product",
            title: product.name,
            subtitle: product.category?.name || "Produto",
            price: product.price,
      ...
      // Add REAL store results (not mock + no fake "Loja verificada")
      if (storeState.stores && storeState.stores.length > 0) {
        storeState.stores.slice(0, MAX_RESULTS_PER_TYPE).forEach((store: any) => {
          searchResults.push({
            id: store.id,
            type: "store",
            title: store.name,
            // 🔒 FIX (Bug #13): Use REAL isVerified field from database instead of hardcoded "Loja verificada"
            subtitle: store.isVerified ? "Loja verificada" : `${store.city}, ${store.state}`,
      ...
      // Add category suggestions if query matches (Bug #14 fix: dynamic categories)
      const categoriesState = useCategoryStore.getState();
      const matchingCategories = categoriesState.categories
      ...
      matchingCategories.forEach((category) => {
        searchResults.push({
          ...
          title: category.name,
          subtitle: "Categoria",
          url: `/produtos?category=${category.slug}`,
        });
      });
  ...
  const handleSearchSubmit = (searchQuery?: string) => {
    ...
    navigate(`/produtos?search=${encodeURIComponent(finalQuery)}`);
  };
