...
export function UserRoleIndicator({
  ...
  if (!showTooltip) {
    return (
      <div
        className={`inline-flex items-center ${sizes.container} rounded-full font-medium border ${config.color} ${className}`}
      >
  ...
  return (
    ...
    >
      <div
        className={`inline-flex items-center ${sizes.container} rounded-full font-medium border ${config.color} cursor-help transition-all duration-200 hover:scale-105 ${className}`}
      >
        <Icon className={`${sizes.icon} mr-1`} />
        {config.label}
      ...
      {isTooltipVisible && (
        <div
          className={`absolute z-50 ${sizes.tooltip} p-4 mt-2 bg-white rounded-lg shadow-xl border border-gray-200 left-1/2 transform -translate-x-1/2`}
        >
          <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 w-4 h-4 bg-white border-l border-t border-gray-200 rotate-45"></div>

          <div className="flex items-center mb-2">
            <Icon
              className={`${sizes.icon} mr-2 ${config.color.includes("red") ? "text-red-600" : config.color.includes("green") ? "text-green-600" : "text-blue-600"}`}
            />
