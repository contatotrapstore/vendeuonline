...
export const useStoreManagementStore = create<StoreManagementStore>((set, get) => ({
  ...
  approveStore: async (storeId: string) => {
    ...
    try {
      ...
      // Refetch stores do servidor para garantir sincronização
      ...
      throw error; // Propagar erro para componente exibir toast
    }
  },
  ...
  suspendStore: async (storeId: string, reason?: string) => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`/api/admin/stores/${storeId}/suspend`), {
        method: "POST",
        headers: authHeaders,
        body: JSON.stringify({ reason }),
      });
  ...
  activateStore: async (storeId: string) => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`/api/admin/stores/${storeId}/activate`), {
        method: "POST",
        headers: authHeaders,
      });
