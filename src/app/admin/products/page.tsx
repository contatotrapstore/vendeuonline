  ...
  const handleDeleteProduct = async (productId: string) => {
    if (confirm("Tem certeza que deseja excluir este produto?")) {
      try {
        ...
        // Refetch produtos do servidor para garantir sincronização (backend faz soft delete)
  ...
  const handleFeaturedToggle = async (productId: string, currentFeatured: boolean) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Erro ao atualizar produto em destaque");
      }

      const result = await response.json();
      if (!result.success) {
        throw new Error(result.error || "Falha ao atualizar produto em destaque");
      }
