...
import { MapPin, Mail, Clock, Users, Award, Target } from "lucide-react";
...
export default function AboutPage() {
  const [stats, setStats] = useState<PlatformStats>({ sellers: 0, products: 0, avgRating: 0 });
  const [loadingStats, setLoadingStats] = useState(true);

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await fetch(buildApiUrl("/api/stats/platform"));
        if (response.ok) {
  ...
  const statsCards = [
    { label: "Vendedores ativos", value: stats.sellers > 0 ? `${stats.sellers}+` : "0", icon: Users },
    { label: "Produtos cadastrados", value: stats.products > 0 ? `${stats.products}+` : "0", icon: Award },
    { label: "Avaliação média", value: stats.avgRating > 0 ? stats.avgRating.toFixed(1) : "0.0", icon: Target },
    { label: "Cidades atendidas", value: loadingStats ? "..." : "Erechim e região", icon: MapPin },
  ];
      ...
      <div className="bg-gradient-to-br from-blue-600 to-purple-600 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Sobre o {APP_CONFIG.name}</h1>
            <p className="text-xl md:text-2xl text-blue-100 max-w-3xl mx-auto">
              Conectando compradores e vendedores em {APP_CONFIG.region.fullName} através de uma plataforma moderna e
              segura.
            </p>
          </div>
        </div>
            ...
            {statsCards.map((stat, index) => {
              ...
              return (
                <div key={index} className="text-center">
                  ...
                  <div className="text-3xl font-bold text-gray-900 mb-2">{loadingStats ? "..." : stat.value}</div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-lg text-gray-600 mb-6">
                Democratizar o comércio eletrônico em {APP_CONFIG.region.fullName}, oferecendo uma plataforma acessível
                e eficiente para que pequenos e médios empreendedores possam expandir seus negócios online.
              </p>
              <p className="text-lg text-gray-600">
                Acreditamos no poder da tecnologia para transformar comunidades e fortalecer a economia local,
                conectando pessoas e criando oportunidades.
              </p>
            </div>
      ...
      <div className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600">Estamos sempre prontos para ajudar você a crescer seu negócio.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
