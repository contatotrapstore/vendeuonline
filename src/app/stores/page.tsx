...
import { STORE_PLACEHOLDER } from "@/lib/constants";
...
export default function StoresPage() {
  ...
  const { stores, loading, error, pagination, fetchStores, setFilters, clearError } = useStoreStore();

  // Carregar lojas quando o componente monta ou filtros mudam
  useEffect(() => {
    // FIX: Validar length mínimo para evitar buscar com 1 char (backend valida >=2)
    const trimmed = searchTerm.trim();
    const validSearch = trimmed.length === 0 || trimmed.length >= 2 ? searchTerm : undefined;
      ...
      <div className="bg-gradient-to-br from-blue-600 to-purple-600 text-white py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Lojas Parceiras</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading && (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Carregando lojas...</h3>
              <p className="text-gray-600">Aguarde enquanto buscamos as melhores lojas para você.</p>
            </div>
          )}
          ...
          {!loading && !error && filteredStores.length === 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
              ...
              <p className="text-gray-600">Tente ajustar seus filtros de busca ou explore outras categorias.</p>
            </div>
          )}
          ...
          {!loading && !error && filteredStores.length > 0 && (
            <div className={viewMode === "grid" ? "grid md:grid-cols-2 lg:grid-cols-3 gap-8" : "space-y-6"}>
              {filteredStores.map((store) => (
                <Link key={store.id} to={`/stores/${store.slug || store.id}`}>
                  <div
                  ...
                  >
                    <div className={viewMode === "list" ? "w-48 flex-shrink-0" : ""}>
                      <img
                        src={store.logo || store.banner || STORE_PLACEHOLDER}
                        alt={store.name}
                        className={`w-full object-cover ${viewMode === "list" ? "h-full" : "h-48"}`}
                        onError={(e) => {
                          const target = e.target as HTMLImageElement;
                          target.src = STORE_PLACEHOLDER;
                        }}
                      />
                    </div>
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg transition-colors ${
                        pageNum === currentPage ? "bg-blue-600 text-white" : "text-gray-600 hover:bg-gray-100"
                      }`}
                    >
                      {pageNum}
                    </button>
          ...
          <div className="mt-16 bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl p-8 text-white text-center">
            <h3 className="text-2xl font-bold mb-4">Quer vender no {APP_CONFIG.name}?</h3>
            <p className="text-blue-100 mb-6 max-w-2xl mx-auto">
              Junte-se às melhores lojas de {APP_CONFIG.region.fullName} e alcance milhares de clientes. Comece com 30
              dias grátis!
            </p>
