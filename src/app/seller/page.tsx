...
export default function SellerDashboard() {
  ...
  if (!user || user.type !== "seller" || loading) {
    return (
            ...
            <div className="flex items-center space-x-4">
              <button
                onClick={() => {
                  ...
                  if (storeIdentifier) {
                    const storeUrl = `${window.location.origin}/stores/${storeIdentifier}`;
              ...
              <button
                onClick={() => {
                  const storeIdentifier = storeSlug || storeId;
                  if (storeIdentifier) {
                    logger.info(`✅ Navegando para: /stores/${storeIdentifier}`);
                    navigate(`/stores/${storeIdentifier}`);
                  } else {
                    logger.info("❌ Store slug/ID não encontrado");
                    alert("Sua loja ainda não foi criada. Configure sua loja primeiro.");
                  }
                }}
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{(stats.storeViews ?? 0).toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-medium text-gray-900">
                          R$ {(product.revenue ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                        </p>
                        <p
                          className={`text-sm ${
                            product.stock === 0
