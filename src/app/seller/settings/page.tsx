...
import {
  ...
  EyeOff,
  CreditCard,
  // Truck removido: Sistema WhatsApp-only (sem gerenciamento de frete)
  DollarSign,
  Package,
...
// ShippingSettings interface removida: Sistema WhatsApp-only (frete negociado via WhatsApp)
...
interface PaymentSettings {
  ...
  // freeShippingMinValue removido: Sistema WhatsApp-only
}
...
export default function SellerSettings() {
  ...
  // Shipping Settings removido: Sistema WhatsApp-only (frete negociado via WhatsApp)
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          // setShipping removido: Sistema WhatsApp-only (frete negociado via WhatsApp)
          setNotifications({
  ...
  // handleShippingSave removido: Sistema WhatsApp-only (frete negociado via WhatsApp)
  ...
  const handlePlanUpgrade = async (planId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(buildApiUrl("/api/sellers/upgrade"), {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const tabs = [
    ...
    // { id: "shipping", label: "Envio", icon: Truck }, // Removido: Sistema WhatsApp-only
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              {/* Shipping Tab removido: Sistema WhatsApp-only (frete negociado via WhatsApp) */}
