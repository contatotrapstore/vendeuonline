...
export const orderSchemas = {
  create: z.object({
    ...
    // shippingAddress removido: Sistema WhatsApp-only (entrega acordada via WhatsApp)
    paymentMethod: z.enum(["CREDIT_CARD", "DEBIT_CARD", "PIX"]),
    notes: z.string().optional(),
  }),

  updateStatus: z.object({
    // Status de entrega removidos (SHIPPED, DELIVERED): Sistema WhatsApp-only
    status: z.enum(["PENDING", "CONFIRMED", "PROCESSING", "CANCELLED"]),
  }),

  // addTracking removido: Sistema WhatsApp-only (rastreamento via WhatsApp)

  query: z.object({
    ...paginationSchema.shape,
    status: z.enum(["PENDING", "CONFIRMED", "PROCESSING", "CANCELLED"]).optional(),
    storeId: z.string().optional(),
    sellerId: z.string().optional(),
  ...
  create: z.object({
    ...
    settings: z
      .object({
        ...
        // freeShippingMinValue removido: Sistema WhatsApp-only (frete acordado via WhatsApp)
