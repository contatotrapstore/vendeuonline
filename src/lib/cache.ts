...
class Cache {
  ...
  // Invalidar cache por pattern (ex: "products:*", "stores:*")
  invalidatePattern(pattern: string): void {
    const keysToDelete: string[] = [];

    // Converter pattern wildcard para regex (products:* → /^products:/)
  ...
  /**
   * 🔒 SECURITY FIX (Bug #6): Generate seller-specific cache key
   * Prevents cache pollution between different sellers
   * @param sellerId - Seller ID for namespace isolation
   * @param baseKey - Base cache key
   * @returns Namespaced cache key
   */
  private getSellerCacheKey(sellerId: string | undefined, baseKey: string): string {
    if (!sellerId) return baseKey;
    return `seller:${sellerId}:${baseKey}`;
  }

  /**
   * 🔒 SECURITY FIX (Bug #6): Set products with seller namespace
   * @param sellerId - Seller ID for isolation
   * @param params - Query parameters
   * @param data - Data to cache
   * @param ttl - Time to live (optional)
   */
  setSellerProducts(sellerId: string | undefined, params: any, data: any, ttl?: number): void {
    const baseKey = this.generateKey("products", params);
    const sellerKey = this.getSellerCacheKey(sellerId, baseKey);
    this.set(sellerKey, data, ttl);
  }

  /**
   * 🔒 SECURITY FIX (Bug #6): Get products with seller namespace
   * @param sellerId - Seller ID for isolation
   * @param params - Query parameters
   * @returns Cached data or null
   */
  getSellerProducts(sellerId: string | undefined, params: any): any | null {
    const baseKey = this.generateKey("products", params);
    const sellerKey = this.getSellerCacheKey(sellerId, baseKey);
    return this.get(sellerKey);
  }

  /**
   * 🔒 SECURITY FIX (Bug #6): Clear all seller-specific cache
   * Called on logout to prevent cache leaks
   * @param sellerId - Seller ID to clear cache for
   */
  clearSellerCache(sellerId: string): void {
    if (!sellerId) return;
    this.invalidatePattern(`seller:${sellerId}:*`);
  }
}

// Instância global do cache
export const appCache = new Cache({
  ttl: 30 * 1000, // 30 segundos (reduzido de 5 min para atualizar produtos aprovados rapidamente)
