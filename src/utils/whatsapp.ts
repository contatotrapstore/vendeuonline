 ...
 * @param deliveryOption - Opção de entrega selecionada (opcional)
...
export function formatSingleProductMessage(
  ...
  let message = `Olá! Vi este produto na *Vendeu Online*:
...
${quantity > 1 ? `💯 *Total:* ${total}` : ""}`;
  ...
  // Adiciona informações de entrega se selecionada
  if (deliveryOption) {
    message += `\n\n🚚 *Entrega:* ${deliveryOption.name}`;
    if (deliveryOption.prazo) {
      message += `\n📅 *Prazo:* ${deliveryOption.prazo}`;
    }
    if (deliveryOption.valor !== undefined && deliveryOption.valor !== null) {
      message += `\n💵 *Frete:* ${deliveryOption.valor === 0 ? "Grátis" : formatPrice(deliveryOption.valor)}`;
    }
  }
...
export function generateProductWhatsAppLink(
  ...
  const message = formatSingleProductMessage(product, store.name, quantity, deliveryOption);
...
export function buyViaWhatsApp(
  ...
  try {
    const whatsappUrl = generateProductWhatsAppLink(product, store, quantity, deliveryOption);
