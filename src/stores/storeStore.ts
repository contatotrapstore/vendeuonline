...
export const useStoreStore = create<StoreState>((set, get) => ({
  ...
  fetchStores: async (filters = {}, page = 1, limit = 10) => {
    try {
      ...
      // Armazenar no cache
      appCache.setStores(cacheParams, response, 30 * 1000); // 30 segundos (reduzido de 5 min para atualizar lojas aprovadas rapidamente)
  ...
  fetchStoreBySlug: async (slug: string) => {
    try {
      ...
      // Backend agora aceita slug direto na rota /:identifier
      const store = await apiGet(`/api/stores/${slug}`);
