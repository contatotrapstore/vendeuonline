...
export async function getPlansAnon() {
  try {
    return await supabaseAnonFetch("plans?is_active=eq.true&order=order.asc");
  } catch (error) {
...
export async function getProductsAnon() {
  try {
    return await supabaseAnonFetch("Product?isActive=eq.true&order=createdAt.desc&select=*,images:ProductImage(*)");
  } catch (error) {
...
export async function getStoresAnon() {
  try {
    return await supabaseAnonFetch("stores?isActive=eq.true&select=*");
  } catch (error) {
...
export async function getTrackingConfigsAnon() {
  try {
    return await supabaseAnonFetch("system_configs?category=eq.tracking&isActive=eq.true&select=key,value,isActive");
  } catch (error) {
