...
function validateAdminAPIs() {
  ...
  ADMIN_APIS_TO_TEST.forEach((api, index) => {
    ...
    if (api.requiresAuth) {
      ...
      statusMessage = statusCode === 401 ? '401 (Auth Required)' :
                     statusCode === 403 ? '403 (Admin Required)' :
                     statusCode === 400 ? '400 (Bad Request)' :
                     statusCode === 422 ? '422 (Validation Error)' :
                     statusCode === 200 ? '200 (OK)' :
                     `${statusCode} (Error)`;
    } else {
      // Para APIs públicas, esperamos 200
      isWorking = statusCode === 200 || statusCode === 400 || statusCode === 422;
      statusMessage = statusCode === 200 ? '200 (OK)' :
                     statusCode === 400 ? '400 (Bad Request)' :
                     statusCode === 422 ? '422 (Validation Error)' :
  ...
  Object.entries(results).forEach(([category, stats]) => {
    ...
    const statusColor = percentage === 100 ? colors.green :
  ...
  const finalColor = totalPercentage === 100 ? colors.green :
                    totalPercentage >= 90 ? colors.yellow : colors.red;

  console.log(`${finalColor}${colors.bold}${workingCount}/${TOTAL_APIS} APIs Admin funcionando (${totalPercentage}%)${colors.reset}`);
